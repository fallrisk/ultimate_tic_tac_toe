...
class Board(object):
  ...
  def is_game_over(self):
    ...
    """
    # Generate a list of the winners of the subboards and check to see if someone
    # won the overall board.
    b = []
    for x in range(9):
  ...
  def get_winner(self):
    ...
    @return Returns 'N' for no winner. 'X' for player X, and 'O' for player O.
    """
    return self._winner
  
  def get_mark(self, cell, subcell):
    """
    Returns the mark at the subcell of the board. Cell is the top layer, and
    subcell is the cell within the cell of the top layer board.
    @return Returns 'E' for empty. 'X' for player X, and 'O' for player O.
    """
